...
CredentialService.getDragonchainId = () => {
    ...
    try {
        ...
        if (e.message === 'MISCONFIGURED_CRED_FILE') {
            throw new FailureByDesign('NOT_FOUND', 'credential file is missing a default id');
        }
        if (e.code === 'ENOENT') {
            throw new FailureByDesign('NOT_FOUND', `credential file not found at "${credentialFilePath}"`);
        }
...
CredentialService.getDragonchainCredentials = (dragonchainId) => {
    ...
    if (dragonchainId === '') {
        throw new FailureByDesign('VALIDATION_ERROR', '"dragonchainId" can not be undefined when checking Dragonchain credential file.');
    }
    // check credential file on disk.
    const credentialFilePath = CredentialService.getCredentialFilePath();
    ...
    try {
        try {
            ...
            if (e.message === 'MISCONFIGURED_CRED_FILE') {
                logger.debug(`credential file is missing a config for ${dragonchainId}`);
            }
            if (e.code === 'ENOENT') {
                logger.debug(`credential file not found at "${credentialFilePath}"`);
            }
        }
        ...
        if (error.code === 'ENOENT') {
            throw new FailureByDesign('NOT_FOUND', 'credentials missing from mounted secrets volume');
        }
        throw new FailureByDesign('UNEXPECTED_ERROR', `Something unexpected happened while looking for credentials at "${credentialFilePath}"`);
    }
};
