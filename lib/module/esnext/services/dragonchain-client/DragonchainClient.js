    ...
    constructor(endpoint, credentials, verify, injected = {}) {
        ...
        this.getSmartContractSecret = async (options) => {
            if (!options.secretName)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `secretName` is required');
        ...
        this.getTransaction = async (options) => {
            if (!options.transactionId)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `transactionId` is required');
        ...
        this.createTransaction = async (options) => {
            if (!options.transactionType)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `transactionType` is required');
            if (!options.payload)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `payload` is required');
        ...
        this.createBulkTransaction = async (options) => {
            if (!options.transactionList)
                throw new FailureByDesign('BAD_REQUEST', 'parameter `transactionList` is required');
        ...
        this.getBlock = async (options) => {
            if (!options.blockId)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `blockId` is required');
        ...
        this.createSmartContract = async (options) => {
            if (!options.transactionType)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `transactionType` is required');
            if (!options.image)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `image` is required');
            if (!options.cmd)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `cmd` is required');
            if (options.scheduleIntervalInSeconds && options.cronExpression)
                throw new FailureByDesign('BAD_REQUEST', 'Parameters `scheduleIntervalInSeconds` and `cronExpression` are mutually exclusive');
            const body = {
                version: '3',
                image: options.image,
                cmd: options.cmd
        ...
        this.updateSmartContract = async (options) => {
            if (!options.smartContractId)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `smartContractId` is required');
            if (options.scheduleIntervalInSeconds && options.cronExpression)
                throw new FailureByDesign('BAD_REQUEST', 'Parameters `scheduleIntervalInSeconds` and `cronExpression` are mutually exclusive');
            const body = {
                version: '3'
        ...
        this.getSmartContract = async (options) => {
            if (options.smartContractId && options.transactionType)
                throw new FailureByDesign('BAD_REQUEST', 'Only one of `smartContractId` or `transactionType` can be specified');
            if (options.smartContractId)
        ...
        this.getSmartContractObject = async (options) => {
            if (!options.key)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `key` is required');
            if (!options.smartContractId) {
                if (!process.env.SMART_CONTRACT_ID)
                    throw new FailureByDesign('BAD_REQUEST', 'Parameter `smartContractId` is required when not running within a smart contract');
        ...
        this.listSmartContractObjects = async (options) => {
            ...
            if (options.prefixKey) {
                if (options.prefixKey.endsWith('/'))
                    throw new FailureByDesign('BAD_REQUEST', 'Parameter `prefixKey` cannot end with \'/\'');
        ...
        this.updateTransactionType = async (options) => {
            if (!options.transactionType)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `transactionType` is required');
            if (!options.customIndexes)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `customIndexes` is required');
            const body = {
                version: '1',
        ...
        this.createBitcoinTransaction = async (options) => {
            if (!options.network)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `network` is required');
            const body = {
        ...
        this.createEthereumTransaction = async (options) => {
            if (!options.network)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `network` is required');
            if (!options.to)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `to` is required');
            if (!options.value)
                throw new FailureByDesign('BAD_REQUEST', 'Parameter `value` is required');
            const body = {
                network: options.network,
                transaction: {
                    to: options.to,
