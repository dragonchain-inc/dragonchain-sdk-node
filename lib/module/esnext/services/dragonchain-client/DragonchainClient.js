...
export class DragonchainClient {
    ...
    constructor(dragonchainId = '', verify = true, injected = {}) {
         ...
         */
        this.getSecret = (secretName) => this.readFileSync(`/var/openfaas/secrets/sc-${process.env.SMART_CONTRACT_ID}-${secretName}`, 'utf-8');
        /**
         ...
         * ```
         */
        this.queryTransactions = async (luceneQuery, sort, offset = 0, limit = 10) => {
        ...
        this.getBlock = async (blockId) => {
            return await this.get(`/block/${blockId}`);
        };
        /**
         ...
         * ```
         */
        this.queryBlocks = async (luceneQuery, sort, offset = 0, limit = 10) => {
            const queryParams = this.getLuceneParams(luceneQuery, sort, offset, limit);
         ...
         */
        this.getSmartContract = async (contractId) => {
            return await this.get(`/contract/${contractId}`);
        };
        /**
         ...
         * ```
         */
        this.querySmartContracts = async (luceneQuery, sort, offset = 0, limit = 10) => {
            const queryParams = this.getLuceneParams(luceneQuery, sort, offset, limit);
         ...
         */
        this.updateSmartContract = async (contractId, image, cmd, executionOrder, desiredState, args, env, secrets, seconds, cron, auth) => {
            const body = {
                version: '3',
            ...
            if (auth)
                body['auth'] = auth;
            return await this.put(`/contract/${contractId}`, body);
        };
        /**
         ...
         */
        this.deleteSmartContract = async (contractId) => {
            return await this.delete(`/contract/${contractId}`);
        };
        /**
         ...
         */
        this.updateMatchmakingConfig = async (askingPrice, broadcastInterval) => {
            if (askingPrice) {
                if (isNaN(askingPrice) || askingPrice < 0.0001 || askingPrice > 1000) {
                    throw new FailureByDesign('BAD_REQUEST', `askingPrice must be between 0.0001 and 1000.`);
                }
            }
            ...
            return await this.put(`/update-matchmaking-data`, matchmakingUpdate);
        };
        /**
         ...
         */
        this.updateDragonnetConfig = async (maximumPrices) => {
            const dragonnet = {};
            [2, 3, 4, 5].forEach(i => {
                const item = maximumPrices[`l${i}`];
                if (item) {
                    if (isNaN(item) || item < 0 || item > 1000) {
                        throw new FailureByDesign('BAD_REQUEST', 'maxPrice must be between 0 and 1000.');
                    }
            ...
            if (Object.keys(dragonnet).length === 0)
                throw new FailureByDesign('BAD_REQUEST', 'No valid levels provided');
            return await this.put(`/update-matchmaking-data`, { dragonnet });
        };
        /**
         ...
         */
        this.createTransaction = async (transactionObject) => {
            return await this.post(`/transaction`, transactionObject);
        };
        /**
         ...
         */
        this.createBulkTransaction = async (transactionBulkObject) => {
            return await this.post(`/transaction_bulk`, transactionBulkObject);
        };
        /**
         * Create a new Smart Contract on your Dragonchain.
         * @returns {Promise<DragonchainContractCreateResponse>}
         */
        this.createContract = async (body) => {
         ...
         * @param {number} level
         */
        this.getVerifications = async (blockId, level = 0) => {
            if (level) {
                return await this.get(`/verifications/${blockId}?level=${level}`);
            }
            return await this.get(`/verifications/${blockId}`);
        };
        /**
         ...
         */
        this.getSmartContractHeap = async (key, scName, jsonParse = false) => {
         ...
         */
        this.listSmartcontractHeap = async (scName, key = '') => {
         ...
         */
        this.registerTransactionType = async (txnTypeStructure) => {
            return await this.post('/transaction-type', txnTypeStructure);
        };
        /**
         ...
         */
        this.deleteTransactionType = async (transactionType) => {
            return await this.delete(`/transaction-type/${transactionType}`);
        };
        /**
         ...
         */
        this.updateTransactionType = async (transactionType, customIndexes) => {
            const params = { version: '1', custom_indexes: customIndexes };
            return await this.put(`/transaction-type/${transactionType}`, params);
        };
        /**
         * @hidden
         * getTransactionType
         ...
         * @param {string} transactionType
         */
        this.getTransactionType = async (transactionType) => {
            return await this.get(`/transaction-type/${transactionType}`);
        };
        /**
         * @hidden
         */
     ...
     * @hidden
     */
    async makeRequest(path, method, body = '', jsonParse = true) {
